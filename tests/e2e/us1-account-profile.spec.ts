...
test('keyboard-only activation, profile, recovery, and deletion journey', async ({ page }) => {
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path.endsWith('/registrations')) {
      await route.fulfill({
        status: 201,
        json: { status: 'pending_verification', emailVerification: 'required' },
      });
      return;
    }
  ...
  await page.getByLabel('Nome').pressSequentially('Pessoa Ficticia');
  await page.getByLabel('E-mail').pressSequentially('jornada.ficticia@example.test');
  await page.getByLabel('Senha', { exact: true }).pressSequentially('Senha-ficticia-123!');
  await page.getByLabel('Confirmar senha').pressSequentially('Senha-ficticia-123!');
  await page.getByLabel(/aceito os termos/i).press('Space');
  await page.getByRole('button', { name: 'Criar conta' }).press('Enter');
  await expect(page.getByRole('status')).toContainText(/confirmacao/i);
  ...
  await page.getByLabel('Nome de exibicao').fill('Pessoa Ficticia');
